...
// Ambil data voucher
$query = "SELECT * FROM voucher_diskon WHERE voucher_id = ? AND user_id_penjual = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($kode) || empty($tipe) || $nilai <= 0) {
        ...
        // Cek kode voucher sudah ada (kecuali kode sendiri)
        $check_query = "SELECT voucher_id FROM voucher_diskon WHERE kode_voucher = ? AND voucher_id != ?";
        $check_stmt = mysqli_prepare($koneksi, $check_query);
        mysqli_stmt_bind_param($check_stmt, "si", $kode, $voucher_id);
        ...
        if (mysqli_stmt_get_result($check_stmt)->num_rows > 0) {
            ...
            // Update voucher
            $update_query = "UPDATE voucher_diskon SET 
                            kode_voucher = ?, tipe_diskon = ?, nilai_diskon = ?, 
                            min_pembelian = ?, max_diskon = ?, kuota_total = ?, 
                            tanggal_mulai = ?, tanggal_berakhir = ?, deskripsi = ?, is_active = ?
                            WHERE voucher_id = ? AND user_id_penjual = ?";
            $update_stmt = mysqli_prepare($koneksi, $update_query);
            mysqli_stmt_bind_param($update_stmt, "ssdddiissiii", 
                $kode, $tipe, $nilai, $min_pembelian, $max_diskon, $kuota, 
                $tgl_mulai, $tgl_berakhir, $deskripsi, $is_active, $voucher_id, $user_id
            );
            
            if (mysqli_stmt_execute($update_stmt)) {
                header("Location: kelola_voucher.php?success=" . urlencode("Voucher berhasil diperbarui!"));
                exit();
            } else {
                $error = 'Gagal memperbarui voucher: ' . mysqli_error($koneksi);
            }
        }
    }
}
...
<head>
    ...
    <title>Edit Voucher - InGrosir</title>
    ...
    <main class="main-content">
        ...
        <div class="form-card">
            <h2 class="mb-4">
                ...
                Edit Voucher: <span class="text-primary"><?php echo htmlspecialchars($voucher['kode_voucher']); ?></span>
            </h2>
            ...
            <form method="POST" onsubmit="return validateForm()">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="kode_voucher" class="form-control" 
                               ...
                               required pattern="[A-Z0-9]+" maxlength="50"
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="tipe_diskon" id="tipe_diskon" class="form-select" required onchange="updateDiskonLabel()">
                            <option value="persentase" <?php echo ($voucher['tipe_diskon'] == 'persentase') ? 'selected' : ''; ?>>Persentase (%)</option>
                            <option value="nominal" <?php echo ($voucher['tipe_diskon'] == 'nominal') ? 'selected' : ''; ?>>Nominal (Rp)</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="nilai_diskon" id="nilai_diskon" class="form-control" 
                               value="<?php echo $voucher['nilai_diskon']; ?>"
                               required min="1" step="0.01">
                        <small class="text-muted" id="diskon_hint">-</small>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="min_pembelian" class="form-control" 
                               value="<?php echo $voucher['min_pembelian']; ?>"
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="max_diskon" class="form-control" 
                               value="<?php echo $voucher['max_diskon']; ?>"
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="kuota_total" class="form-control" 
                               value="<?php echo $voucher['kuota_total']; ?>"
                               placeholder="Kosongkan untuk unlimited" min="<?php echo $voucher['kuota_terpakai']; ?>">
                        <small class="text-muted">Sudah digunakan: <?php echo $voucher['kuota_terpakai']; ?>x</small>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="datetime-local" name="tanggal_mulai" class="form-control" 
                               value="<?php echo date('Y-m-d\TH:i', strtotime($voucher['tanggal_mulai'])); ?>"
                               required>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="datetime-local" name="tanggal_berakhir" class="form-control" 
                               value="<?php echo date('Y-m-d\TH:i', strtotime($voucher['tanggal_berakhir'])); ?>"
                               required>
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="deskripsi" class="form-control" rows="3"><?php echo htmlspecialchars($voucher['deskripsi']); ?></textarea>
                    </div>
                    ...
                    <div class="col-12">
                        <div class="form-check form-switch">
                            ...
                            <label class="form-check-label fw-bold" for="is_active">
                                <i class="fas fa-toggle-on me-1"></i>Voucher Aktif
                            </label>
                            <small class="d-block text-muted">Nonaktifkan untuk pause voucher sementara</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="alert alert-warning mt-4">
                    ...
                    <strong>Perhatian:</strong> Perubahan tidak akan mempengaruhi pesanan yang sudah menggunakan voucher ini.
                </div>
